...
/**
 * A swappable AI provider (docs/29 §3, ADR-005). Implementations:
 *   - `MockAIProvider` — deterministic, offline, the dev/CI default;
 *   - `OpenAICompatibleProvider` — a generic OpenAI-compatible endpoint,
 *     constructed only server-side, with an injected `fetch`.
