...
describe("DiscoveryProviderRegistry", () => {
  ...
  it("resolves EUROPE_PMC when an injected fetch is supplied", () => {
    const registry = createDefaultDiscoveryRegistry();
    const { fetch } = makeEuropePMCFixtureFetch({});
    const provider = registry.resolve("EUROPE_PMC", { fetch });
    expect(provider).toBeInstanceOf(EuropePMCDiscoveryProvider);
